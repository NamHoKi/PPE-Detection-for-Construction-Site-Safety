...
class MyWindow(QMainWindow):
    ...
    def setupUI(self):
        self.config_file = './configs/dynamic_rcnn/dynamic_rcnn_r50_fpn_1x_coco.py'
        self.cfg = Config.fromfile(self.config_file)
        self.cfg.model.roi_head.bbox_head.num_classes = 7
        self.cfg.model.rpn_head.anchor_generator.scales = [4]
        self.cfg.load_from = "./dynamic_rcnn_r50_fpn_1x-62a3f276.pth"
        self.cfg.lr_config.warmup = None
        self.cfg.evaluation.metric = 'bbox'
        self.cfg.evaluation.iterval = 10
        self.cfg.checkpoint_config.interval = 10
        self.cfg.log_config.interval = 10
        self.cfg.runner.max_epoch = 10
        self.cfg.seed = 0
        self.cfg.gpu_ids = range(1)
        # self.config_file = 'configs/faster_rcnn/faster_rcnn_r50_fpn_1x_coco.py'
        self.checkpoint_file = './work_dirs/0306/epoch_80.pth'
        # self.checkpoint_file = 'checkpoints/faster_rcnn_r50_fpn_1x_coco_20200130-047c8118.pth'
        self.model = init_detector(self.cfg, self.checkpoint_file)
        ...
        # Button 2 - Removed
        btn3 = QPushButton("mmdetect", self)
        btn3.setStyleSheet("color: white;"
                           "background-color: #444444")
        btn3.move(10, 80)
        btn3.resize(130, 50)
        btn3.clicked.connect(self.showFile)
    ...
    def showFile(self):
        ...
        if fname[0] != '' and fname[0][-4:] == '.mp4':
            ...
            video_reader = mmcv.VideoReader(fileName)

            for frame in mmcv.track_iter_progress(video_reader):
                result = inference_detector(self.model, frame)
                frame = self.model.show_result(frame, result, score_thr=0.4)
                if not self.mp4_stop:
                    ...
                    self.mp4_stop = not self.mp4_stop
                    break
